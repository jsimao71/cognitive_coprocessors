...
set "RUN=%DATA%\replications\seed23\qwen_run"
set "EVALUATION=%DATA%\replications\seed23\eval"
...
if not exist "%RUN%\training_report.json" (
    "%PYTHON%" -u -m ccpu paper1 train-lora ^
        --config "%REPO_ROOT%\configs\paper1\e3_d1_f0_l0_qwen_lora_xpu_seed23.json" ^
...
if not exist "%EVALUATION%\summary.json" (
    "%PYTHON%" -u -m ccpu paper1 run-asl-pilot ^
        ...
        --adapter-id "Qwen3-0.6B-E3-D1-F0-L0-r8-seed23" ^
        --condition lora ^
        --shots 0 ^
