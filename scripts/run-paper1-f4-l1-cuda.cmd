...
set "DATA=%REPO_ROOT%\artifacts\paper1\e3_v2\bottleneck_v1\sft"
set "OUTPUT=%REPO_ROOT%\artifacts\paper1\e3_v2\m2_f4_semantic_weighted_cuda"
...
if not exist "%RUN%\training_report.json" (
    "%PYTHON%" -u -m ccpu paper1 train-lora ^
        --config "%REPO_ROOT%\configs\paper1\e3_f4_l1_qwen_lora_cuda.json" ^
...
if not exist "%EVALUATION%\summary.json" (
    "%PYTHON%" -u -m ccpu.paper1.e3 run-bottleneck ^
        ...
        --adapter-id "Qwen3-0.6B-E3-F4-L1-r8-seed11-cuda" ^
        --objective-id L1 ^
