...
\title{Transactional Cognitive State: Hypotheses, Provenance, Retraction, and Backtracking}
...
\begin{itemize}
\item Add epistemic/provenance classes.
\item Implement transactions/scopes, dependencies, retraction, and bounded rollback.
\item Create hypothesis-testing and revision tasks.
\item Compare monotonic versus transactional memory.
\item Measure contamination, recovery, rollback correctness, state size, and reliability.
\item Evaluate automatic counterexample feedback.
\end{itemize}
...
\begin{itemize}
\item Do not equate formal derivability with truth.
\item Keep rollback semantics auditable.
\item No broad search-tree claims without direct evidence.
\end{itemize}
