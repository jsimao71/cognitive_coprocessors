...
\title{Learned Semantic Interrupts for Cognitive Coprocessors}
...
\begin{itemize}
\item Create event datasets with paraphrases and hard contextual negatives.
\item Separate DETECT, NORMALIZE, EXECUTE.
\item Compare grammar, heuristic NLP, small learned parser/classifier, and optional frozen-model hidden-state probes.
\item Add confidence, abstention, calibration, and fallback explicit tools.
\item Evaluate OOD paraphrases and model-size sensitivity.
\end{itemize}
...
\begin{itemize}
\item No joint base-model training.
\item No primary backtracking contribution.
\item Never silently promote uncertain language to hard fact.
\end{itemize}
