...
Automatic Calculator Assistance During Autoregressive Decoding}
...
We study a narrower alternative: a bounded calculator watches the generated
character stream for completed strict arithmetic syntax, normalizes the
expression into typed postfix micro-IR, evaluates it exactly, and inserts the
result immediately after the completing equals sign. The model need not emit a
function name or API schema. This first experiment is intentionally small: it
uses integer literals, an allowlisted operator grammar, append-only trace state,
and textual reinjection; it includes no learned trigger, general symbolic
reasoner, backtracking, or native key/value interface. We define matched
model-only, prompted, explicit-tool, reflex, and oracle conditions; a factorial
sweep over expression depth, operand width, model size, and seed; component and
end-to-end metrics; adversarial non-trigger controls; and precise falsification
criteria. The runtime, generator, replay path, optional Hugging Face adapter,
and test suite are implemented. At this protocol revision no language-model
experiment has been run, so we report no empirical performance claim.
\end{abstract}

\section{Status and Scope}

This document is an implemented experimental protocol, not a completed result.
...
The scope is calculator only and strict syntax only. The experiment does not
test semantic recognition of prose arithmetic, learned routing, persistent
...
Reflex assistance reduces model-authored tool-selection and argument tokens
...
\subsection{Factorial arithmetic generator}

The deterministic generator crosses operator count with operand digit width and
draws operators from addition, subtraction, multiplication, and exact rational
division. Expressions are parenthesized left-compositions so the gold parse is
unambiguous. Each cell uses identity-derived random seeds, and gold answers are
produced by a separate exact-rational reference evaluator rather than the
calculator implementation under test. The primary
configuration contains
\[
4\ \text{operator counts}\times
4\ \text{digit widths}\times
100\ \text{examples}=1600
\]
arithmetic examples plus 200 non-trigger controls.
...
Report candidate count, trigger precision/recall, false-intervention rate,
normalization acceptance/correctness, engine success/correctness, reinjection
success, intervention count, and state-item count. Oracle differences localize
missed or malformed strict events.
...
cost. Finally, exact calculator output cannot repair an incorrectly copied
...
The implementation is complete at protocol level; model sweeps and replicated
analysis remain required before any positive or negative architecture claim.
