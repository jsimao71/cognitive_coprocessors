...
rank-8 LoRA adapters were then trained for Qwen3-0.6B and an ungated second family,
SmolLM2-1.7B. With a minimal instruction, both adapters reached 100\% block
execution and answer accuracy with zero false blocks; both unadapted models under
that instruction reached 0\% execution. Frozen ICL reached 100\%/93.8\% execution
but falsely blocked 2/1 controls. These developmental results favor a hybrid:
store stable semantic selection and serialization in small adapter weights, keep
exact execution and enforcement in the runtime, and use context for cold-start
interfaces. The sample remains too small for a general scaling or family claim.
\end{abstract}
...
The XPU training gate completed PEFT forward, backward, and AdamW steps on both
...
with two epochs and assistant-target-only loss. Qwen3-0.6B is primary. Since
official Llama 3.2 and Gemma 3 files remain license-gated without a Hugging Face
token, the predeclared ungated second family is the official Apache-2.0
HuggingFaceTB SmolLM2-1.7B-Instruct checkpoint, not a community mirror.
...
\paragraph{Second-family replication and placement answer.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../../../artifacts/paper1/lora_protocol/placement_comparison_v1/placement_reliability_cost.png}
\caption{Interface success and answer accuracy (left), false interventions
(center), and recurring prompt-plus-output tokens (right). Adapter+minimal is
the only learned placement with perfect execution, answers, and control safety
in both tested families.}
...
these two models. New or changing interfaces can begin in context without
training, but demonstrations recur on every request and produced more false
interventions; combining ICL and LoRA did not improve the primary result. Parsing,
resource bounds, exact arithmetic, reinjection, and enforcement must remain in
the runtime. This division stores policy in weights and mechanism in the runtime,
with context serving development and cold-start use. It is a developmental
design result, not evidence that all interface knowledge should be fine-tuned.
...
behind manual authorization gates; SmolLM2 supplies one ungated family replication
but cannot establish effects for the originally targeted families. The adapter
...
predeclared gate, but the two-family comparison remains developmental rather than
...
two small-model families. In both, adapter+minimal reaches perfect execution,
answers, result use, and observed control safety, while base+minimal never opens
a valid arithmetic block. Context enables cold-start behavior but costs more
tokens and produces false interventions; adding it after adaptation is redundant
or harmful. The normalized runtime is safe and training-free but remains limited
by model expression exposure and result integration. The resulting architecture
is hybrid: context for rapid development, weights for a stable semantic contract,
and the runtime for typed parsing, bounds, exact computation, and enforcement.
Powered confirmatory data and authorized Llama/Gemma runs remain necessary before
stronger placement or family claims.
