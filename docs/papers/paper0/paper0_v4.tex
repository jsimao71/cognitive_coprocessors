...
The stable model-facing contract is deliberately coarse:
\textsc{none}, \textsc{compute}, \textsc{retrieve}, \textsc{verify}, or
\textsc{help}. A deployment-specific runtime resolves that intent to a
capability and implementation. Engine-specific tokens, adapters, fixed
registries, and Progressive Retrieval Attention (PRA) are optional deployment
choices rather than defining requirements.
...
\subsection{Stable cognitive intent and dynamic capability resolution}

The model need not identify an engine. We factor routing into
\[
R_1:(y_{1:t},h_t,\cstate_t)\rightarrow
\{\textsc{none},\textsc{compute},\textsc{retrieve},\textsc{verify},\textsc{help}\}
\]
and
\[
R_2:(d_t,\text{task},\text{registry},\text{policy},\cstate_t)
\rightarrow \text{capability or shortlist}.
\]
\textsc{Compute} requests deterministic or structured work such as arithmetic,
dates, units, graph closure, logic, algebra, or analytics. \textsc{Retrieve}
requests external evidence or data. \textsc{Verify} asks the runtime to check
an existing candidate using evidence, recomputation, logic, provenance, or
active state. \textsc{Help} is a fail-safe cognitive page fault: assistance is
needed, but the model cannot reliably choose a family. \textsc{None} preserves
ordinary generation.

$R_1$ is small and intended to remain stable across deployments. $R_2$ may use
deterministic rules, lexical or token-aware BM25, CPU classifiers, capability
retrieval, optional learned routing, or conventional tools. It selects semantic
capabilities---for example arithmetic, Horn entailment, structured aggregate,
or current factual lookup---rather than treating one backend name as the task.
The runtime then chooses an implementation using exactness, available state,
latency, cost, policy, credentials, and provenance requirements. Adding an
engine should therefore not require retraining $R_1$.

Three surface syntaxes remain empirical alternatives. Paired tags such as
\texttt{<COMPUTE>} payload \texttt{</COMPUTE>} provide explicit incremental
boundaries. A fenced \texttt{COMPUTE} block is familiar and potentially
shorter but can collide with ordinary Markdown. A label-only form emits only
the intent and lets the runtime reuse the active task as payload. Papers 2 and
2.5 measure tokenizer cost, parse reliability, and accidental activation rather
than declaring a universal winner here.

\subsection{Interoperable deployment modes}

The architecture admits the modes in Table~\ref{tab:deployment-modes}. They can
be composed; they are not incompatible product editions.

\begin{table}[t]
\centering\small
\caption{Deployment modes share the coarse intent and typed-state contracts.}
\label{tab:deployment-modes}
\begin{tabularx}{\linewidth}{@{}p{0.28\linewidth}Y@{}}
\toprule
Mode & Model/runtime boundary \\
\midrule
Runtime-trigger coprocessor & No model modification; transparent detector emits intent. \\
In-context coprocessor & A short prompt teaches the generic-intent contract. \\
Generic adapter & Optional LoRA/SFT accelerates intent and result consumption. \\
PRA + in-context & PRA materializes only relevant capability or skill interfaces. \\
PRA + adapter & Learned coarse intent combines with dynamic discovery and state. \\
PRA only & Progressive materialization without a learned intent adapter. \\
Coprocessor only & Conventional registry insertion without PRA. \\
Conventional tools & Universal explicit fallback and matched baseline. \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Three knowledge layers and optional PRA}

The design separates (1) stable cognitive intent, which may live in a prompt,
adapter, or native model; (2) dynamic capability knowledge, which belongs to the
runtime registry, skill system, or PRA resources; and (3) cognitive state,
including typed results, evidence, provenance, dependencies, and task scope.
Conflating these layers would bake a changing product registry into model
weights.

PRA is optional and orthogonal. Without PRA, the registry remains outside the
prompt and the runtime inserts selected capability metadata conventionally.
With PRA, capability descriptions and prior cognitive records can remain
persistent but unmaterialized; task-aware retrieval exposes only a relevant
shortlist and suppresses unrelated historical state. Thus a large registry
need not imply a large active context. An adapter is likewise an optional
accelerator for coarse intent, not the registry itself; engine-specific adapters
remain possible optimizations, never a prerequisite for adding a coprocessor.
...
Neither those blocks nor capability-specific tokens define a cognitive
coprocessor: they are candidate surfaces beneath the stable $R_1/R_2$
factorization.
...
2 & Heterogeneous compute & Does generic \textsc{compute}/\textsc{verify}/\textsc{help} survive registry expansion while runtime capability routing remains exact? & Prefer registry-independent intent; learn engine identity only for measured residual value. \\
2.5 & Heterogeneous retrieval & Does generic \textsc{retrieve}/\textsc{verify}/\textsc{help} survive backend changes while source-native semantics beat matched universal retrieval? & Decouple model intent from the evolving data registry. \\
...
The preferred scalable boundary is therefore a coarse cognitive interrupt,
deployment-specific runtime or PRA capability resolution, specialized
execution, typed cognitive state, and selective continuation. LoRA may make
that interrupt cheaper or more reliable, but it should not become a catalog of
engines. Conventional tools remain the fallback when automatic intent or
runtime resolution is uncertain.
