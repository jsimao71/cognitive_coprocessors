...
\title{Learning to Offload: Co-Adaptation Between Transformers and Cognitive Coprocessors}
...
\begin{itemize}
\item Train matched with/without-coprocessor regimes.
\item Measure offloading, redundant computation, result use, and robustness.
\item Probe learned representations cautiously.
\item Sweep model sizes.
\item Ablate delayed/noisy/incorrect coprocessor outputs and coprocessor removal.
\end{itemize}
...
\begin{itemize}
\item No mechanistic capacity-reallocation claim without probes.
\item Keep engine correctness separate from interface learning.
\item Always evaluate no-coprocessor fallback.
\end{itemize}
