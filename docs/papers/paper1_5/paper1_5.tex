...
64-example test, shallow lexical
accuracy is at most 85.94\%. The unchanged semantic policy reaches 100\% recall
...
supports transparent runtime enforcement, but the still-easy lexical screen and
small synthetic source limit the semantic-generalization claim.
\end{abstract}
...
examples. There is one seed per model and no ranking noise. The natural extension
passes its lexical screen only narrowly and its long-form component is a
deterministic controller simulation, not unconstrained generation. The semantic
cue distribution remains controlled. The runtime gate is
...
controlled one-source regime and retains full recall under a more varied, though
still lexically easy, natural-language freeze. Advisory and instructed evidence
...
semantic selection should remain transparent until richer independently frozen
training demonstrates learned-policy generalization.
