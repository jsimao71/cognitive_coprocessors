...
Textual schemas remain safe but inert.  Thus exact execution belongs in the
runtime, but this experiment does not establish a reliable home for broad
semantic selection.  The preregistered Paper 3 gate is therefore \texttt{no\_go}.
\end{abstract}
...
Every run stores predictions, factorized summaries, dataset and artifact hashes,
pinned model metadata, package/device provenance, and the dirty Git revision at
execution time.  The analysis artifact hashes all 22 source summaries.
