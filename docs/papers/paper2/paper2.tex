...
\begin{itemize}
\item Retain calculator; add Horn/Datalog logic, ISA/frame graph reasoning, and optionally simple constraints.
\item Define common typed micro-IR and provenance.
\item Implement bounded persistent state and forward closure.
\item Create mixed-engine tasks.
\item Compare no-engine, explicit tools, single engines, multiple engines, and oracle selection.
\item Measure scaling with depth, entities, rules, distractors, state size, tokens, and latency.
\end{itemize}
...
\begin{itemize}
\item No learned trigger.
\item Use a controlled FOL/Horn fragment, not unrestricted theorem proving.
\item No rollback/backtracking.
\item No PRA dependency.
\end{itemize}
