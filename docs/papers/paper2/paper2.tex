...
and result use remain unresolved. A tokenizer-aware extension raises the best
development-selected six-way CPU test accuracy to 0.809; a hierarchical
logic-cue/token route plus Qwen fallback avoids 77.4\% of model calls and reaches
0.891 accuracy with zero false activation, but only 0.804 positive engine
selection. Factorized Qwen router LoRAs improve selection
...
but a 24-row authority subset finds only 0.042 effects from masking the result or
removing the distractor and no question/result-competition support.  Provenance
bias remains unlicensed.  A registered 2,253-row public suite first exposes a
formalization/backend gap.  We then freeze an executable 60-row slice spanning
GSM8K, units, dates, ProofWriter, and CLUTRR.  Qwen makes no valid voluntary
four-tool calls; a disjoint shared-token BM25 router selects all 60 engines, but
neural result integration preserves only 41 exact results.  Direct runtime-copy
is 60/60 with zero model calls.  Thus public transfer identifies both routing
and result-use failures; the Paper 3 gate remains \texttt{no\_go}.
\end{abstract}
...
\subsection{Registered public-validation freeze}

To separate synthetic-interface diagnosis from external validity, we register a
public suite before running any new model condition.  It retains 500 GSM8K test
items, 600 BIG-bench unit-conversion validation items, all 73 BIG-bench
date-understanding validation items, all 360 items from a balanced ProofWriter
test freeze, and 720 CLUTRR test items: 2,253 items in total.  Sampling is
deterministic and stratified by arithmetic-step count, linear/rate/powered unit
form, temporal-cue count, ProofWriter proof depth 0--5, and CLUTRR relation depth
2--10.  Repository revisions, converted-Parquet checksums, source row numbers,
labels, difficulty metadata, and content hashes are immutable.  Benchmark text
is not redistributed.

This registration is an evaluation protocol, not a result.  Every later
condition must use the same selected IDs and the same backend within each task.
It must factor assistance need, intent, engine choice, formalization, execution,
result integration, and final answer; report depth curves rather than a pooled
average; and compare generic tools, CPU routing plus exact execution, TwIL, and
oracle formalization where meaningful.
...
\subsection{Token-space triggers and hierarchical CPU routing}

The shallow audit can understate lexical strength because its representation is
fixed. We compare current words and character $n$-grams, one shared Unicode
word/punctuation tokenizer, and raw and normalized pieces from the exact pinned
Qwen, SmolLM2, and Gemma tokenizers. Each token representation receives separate
unigram and unigram--bigram TF--IDF classifiers, BM25 exemplar routing, and an
IDF-weighted class prototype. BM25 searches $k\in\{1,3,5,10\}$ and a trigger
threshold on development only. All conditions add zero context tokens;
artifacts retain tokenized test rows, confusion matrices, per-engine scores,
index sizes, and CPU latency.

\begin{table}[ht]
\centering\scriptsize
\caption{Tokenizer-aware six-way test results on the same 900 rows. Acc.=all-row
accuracy, Sel.=positive engine identity, FAR=control activation, Run.=positive
runtime exactness. Latency includes representation and CPU routing.}
\label{tab:tokenizer-ladder}
\resizebox{\linewidth}{!}{%
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
Condition & Acc. & Macro F1 & Sel. & FAR & Run. & CPU $\mu$s\\
\midrule
...
Native tokenization is stronger than the original learned word baseline, but it
does not dominate the transparent T1 route or clear the 0.9 selection gate.
Qwen-piece BM25 reaches 0.870 positive selection and 0.970 answer-equivalent
runtime exactness, but activates on 20\% of controls and costs 6.3~ms per query.
This is still far below the 554.7~ms Qwen-L2 route, but it is not safe enough.

We also evaluate the proposed hierarchy without merging unmatched TwIL data.
Transparent graph/Datalog cues first defer logic-like prompts; a
development-selected normalized Gemma unigram classifier handles only
NONE/calculator/date/units above its fitted confidence threshold; all remaining
rows use the matched Qwen-L2 router. The CPU accepts 79.56\% of development rows
at 96.65\% accepted accuracy. On test it avoids 77.44\% of model calls and keeps
FAR at zero, but reaches only 0.804 positive engine selection: graph recall is
0.500, versus 1.000 Datalog, 0.890 calculator, 0.990 date, and 0.640 units.
Runtime exactness is 0.904 because some wrong engine identities are
answer-equivalent. The architecture is economically promising but fails the
identity/reliability gate; answer-only scoring would conceal that failure.

\subsection{Public routing and backend-coverage diagnostic}

Table~\ref{tab:public-coverage} applies the frozen T1/T2 rules without tuning
and audits whether each public row supplies executable formalization for the
current backend.  This is deliberately not an LLM accuracy table.  T1/T2 pooled
engine recall falls to 0.139/0.016.  CLUTRR supplies machine-readable relation
paths, but ISA/frame closure is the wrong algebra.  BIG-bench and ProofWriter
provide answers or controlled English rather than our typed IR.  GSM8K is the
only immediately executable bridge: 493/500 selected rows have annotated
arithmetic traces, 368 fit the bounded integer-expression contract, and every
compatible trace executes exactly.  The remaining rows primarily contain
decimal literals (100), annotation-only values without a binary operation (25),
or no annotated trace (7).

\begin{table}[ht]
\centering\scriptsize
\caption{Public-suite transfer audit. Formal is machine-readable oracle
formalization coverage; Backend is compatibility with the current exact
contract; Exec. is exactness conditional on compatible execution.}
\label{tab:public-coverage}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
Task & $n$ & T1 recall & T2 recall & Formal & Backend & Exec.\\
\midrule
GSM8K & 500 & .170 & .034 & .986 & .736 & 1.000 (368)\\
BIG-bench units & 600 & .235 & .030 & .000 & .000 & --\\
BIG-bench dates & 73 & .000 & .000 & .000 & .000 & --\\
ProofWriter & 360 & .000 & .000 & .000 & .000 & --\\
CLUTRR & 720 & .122 & .000 & 1.000 & .000 & --\\
\midrule
All & 2253 & .139 & .016 & .538 & .163 & 1.000 (368)\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.82\linewidth]{paper2_public_coverage.png}
\caption{A correct assistance intent is not enough: transferred routing and
exact-backend coverage remain distinct bottlenecks on public tasks.}
\label{fig:public-coverage}
\end{figure}

This audit changes the next step.  Training a public six-way router before
building matched task adapters would optimize engine labels against backends
that cannot consume four of the five public task representations.  The correct
order is to freeze adapters and backend coverage first, then compare generic
tools, CPU routing, TwIL, hybrids, and oracle formalization on identical rows.

\subsection{Executable public transfer and generic-tool comparison}

We implement bounded adapters before adding model-facing scores.  From the
registered suite, exact validation admits 343/500 GSM8K rows, 32/600 linear unit
conversions, 25/73 date questions, 193/360 attribute-only ProofWriter theories,
and 720/720 CLUTRR rows.  We freeze 12 rows per task.  GSM8K re-executes each
annotated operation through the bounded calculator; units and dates use narrow
prompt parsers; ProofWriter executes parsed controlled English in the Horn
engine.  CLUTRR is explicitly weaker: it replays the dataset's annotated proof
state and is not an execution of the paper's ISA/frame algebra.  Every retained
adapter result is checked against the frozen target before a model condition can
run.

All model conditions use Qwen3-0.6B at pinned revision
\texttt{c1899de289a04d12100db370d81485cdf75e47ca}, one seed, greedy decoding,
and the same 60 IDs.  The skeptical tool prompt exposes exactly
\texttt{\_\_compute}, \texttt{\_\_retrieve}, \texttt{\_\_verify}, and
\texttt{\_\_help}; their fixed schemas cost 72 word tokens and dispatch to the
same registered results as the generic CogCop token.  T1 is the unchanged
regex route.  The stronger CPU route fits shared Unicode unigram--bigram BM25
with $k=3$ on 2,184 other registered rows, excluding all 60 evaluation IDs and
nine duplicate rows sharing those IDs.  It selects 60/60 engines on this slice.
This is within-suite lexical routing without negative controls, not evidence of
open-domain semantic detection.

\begin{table}[ht]
\centering\scriptsize
\caption{Matched executable public results ($n=60$). Select is exact registered
engine selection; Use is final exactness conditional on delivered assistance.
Tokens and calls are generated means.}
\label{tab:public-executable}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
Condition & Final & Select & Assist & Use & Tokens & Calls\\
\midrule
LLM only & .183 & -- & .000 & -- & 11.37 & 1.000\\
Four generic tools & .150 & .000 & .000 & -- & 13.40 & 1.000\\
Generic CogCop token & .150 & .017 & .017 & 1.000 & 34.52 & 1.017\\
CPU T1 & .200 & .050 & .050 & .667 & 9.35 & 1.000\\
CPU BM25 + exact result & .683 & 1.000 & 1.000 & .683 & 7.53 & 1.000\\
Oracle route + exact result & .683 & 1.000 & 1.000 & .683 & 7.53 & 1.000\\
Runtime-copy & 1.000 & 1.000 & 1.000 & 1.000 & 0 & 0\\
\bottomrule
\end{tabular}
\end{table}

Qwen emits no valid four-tool call and one valid generic CogCop token.  Relative
to those 60 voluntary misses, T1 correctly rescues 2 (Automatic Rescue Rate
.033), whereas BM25 plus the registered result rescues 41 (.683).  BM25 and
oracle routing produce identical prompts and all 60 generated answers, so their
equal final score is a matched control.  Their 95\% Wilson interval is
[.558,.787].  Against LLM-only, BM25 has 30 paired gains, no losses, and 30 ties.
The remaining 19 errors occur after an exact result is available: 3 GSM8K, 2
units, 3 dates, 4 ProofWriter, and 7 CLUTRR results are overridden.  Runtime-copy
removes that neural integration step and is exact on every task.

\begin{table}[ht]
\centering\scriptsize
\caption{Final exact accuracy by public task.  The BM25 and oracle columns are
identical; only BM25 is shown.}
\label{tab:public-by-task}
\begin{tabular}{@{}lrrrr@{}}
\toprule
Task & LLM & Four tools & CPU BM25 & Runtime-copy\\
\midrule
GSM8K & .000 & .000 & .750 & 1.000\\
BIG-bench units & .167 & .167 & .833 & 1.000\\
BIG-bench dates & .083 & .000 & .750 & 1.000\\
ProofWriter & .583 & .583 & .667 & 1.000\\
CLUTRR & .083 & .000 & .417 & 1.000\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.96\linewidth]{paper2_public_compute_accuracy.png}
\caption{Matched public final accuracy.  Perfect registered routing does not
guarantee that Qwen preserves an exact result; direct runtime-copy does.}
\label{fig:public-compute}
\end{figure}
...
\subsection{Attention and causal-content diagnostic}

We select 12 authority-formatted INTERPRET and 12 CONTINUE rows, stratified by
prior correctness where available.  Eager Qwen inference retains attention mass
to QUESTION and RESULT spans for every generated token, layer, and head (28
layers, 16 heads), then aggregates only for reporting.  Raw attention is treated
as diagnostic, never causal proof.  Two matched content ablations replace the
exact result with \texttt{[MASKED RESULT]} or remove the planted draft
distractor.  These are causal input ablations, not direct attention-logit masks.

\begin{table}[ht]
\centering
\caption{Bounded authority-format diagnostic ($n=24$). Effects are relative to
full context; the preregistered competition threshold is 0.10 in both directions.}
\label{tab:attention-diagnostic}
\begin{tabular}{lrr}
\toprule
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.94\linewidth]{paper2_attention_causal_diagnostic.png}
\caption{Left: strict causal-content ablations. Right: generated-token attention
mass by outcome. Span lengths differ, so question-versus-result heights are not
directly comparable; correct-versus-incorrect differences within a span are the
relevant diagnostic.}
\label{fig:attention-diagnostic}
\end{figure}

Ten full-context rows are correct and 14 incorrect.  Correct rows allocate mean
attention mass 0.1483 to the question span and 0.0080 to the result; incorrect
rows allocate 0.1218 and 0.0326.  Thus errors do not exhibit a simple lack of
result attention: result mass is higher in incorrect generations.  Result
masking and distractor removal each change exactness by only one row.  The joint
question/result-competition criterion is not met, so fixed provenance bias,
layer-specific beta, and learned provenance/KV embeddings are deferred.  This
negative gate prevents an attention mechanism from being inferred merely from
poor result use.
...
\subsection{Four generic tools: transport-equivalence checkpoint}

We add the fixed gateways \texttt{\_\_compute}, \texttt{\_\_retrieve},
\texttt{\_\_verify}, and \texttt{\_\_help}. Concrete engines remain outside
the model-facing schemas. The gateway validates coarse intent and enters the
same R2 parser, engine registry, policy, typed state, provenance, and result path
as the generic CogCop interface.

As a first invariant, we replay the 100-row five-engine test freeze with oracle
timing. Eighty rows require assistance and 20 are controls. A generic
\texttt{\_\_compute} call carrying the same marked payload and a direct CogCop
block produce identical backend results on all 100 rows; both transports score
1.000 under deterministic runtime-copy. The four schemas occupy 72 lexical
tokens and remain exactly constant as the registry grows from one to five
engines.

This is not a voluntary-tool result. No model chooses whether or when to call,
tool arguments are oracle payloads, and Automatic Rescue Rate is undefined.
The result establishes backend parity and registry/schema invariance so later
model-facing comparisons cannot weaken the skeptical tool baseline. Claims
about autonomy, malformed arguments, JIT timing, CONTINUE, latency, or rescue
remain pending.
...
\item H10 (question/result attention competition explains non-copy failure) is
      not supported by the bounded diagnostic; both causal-content effects are
      0.042 and incorrect rows have higher result-span attention mass.
\end{itemize}
...
The tokenizer-aware hierarchy likewise remains below 0.9 positive selection,
despite zero FAR and substantial model-call avoidance; it does not reopen the
Paper 3 gate.
The oracle-timed four-tool transport audit also leaves the gate unchanged:
backend equivalence is a required control, not evidence of learned assistance.
...
calling. The four-tool audit uses fixed native-like schemas but oracle timing
and arguments; it does not measure voluntary calls, Automatic Rescue Rate,
malformed arguments, or extra model turns. Strict exact matching penalizes semantically correct capitalization or
...
The hierarchy uses the matched Qwen-L2 router rather than TwIL because the TwIL
reasoning run has no row-aligned train/development/test counterpart on this
900-row freeze. Combining those scores would be invalid. The logic-cue prefilter
also misses half of graph cases under the selected hierarchy, so broader
natural-language graph cues or an independently frozen ambiguity classifier are
still required.

The public model-facing result is a 60-row executable slice, not the full
2,253-row registration.  The units and date parsers cover only 32 and 25 source
rows, respectively, and the ProofWriter parser is bounded to attribute theories.
GSM8K uses gold arithmetic annotations and CLUTRR replays gold proof states, so
their formalization scores are oracle-backed rather than end-to-end extraction.
BM25 sees other examples from the same five benchmark distributions and has no
no-assistance controls; its perfect engine routing therefore measures
benchmark-family separability, not safe open-domain triggering.  One checkpoint,
one seed, and 12 rows per task leave wide intervals and do not support model
rankings or depth-curve claims.  The actual four-tool condition is textual rather
than a provider-native function-calling API, although it does execute the same
fixed gateway contract when called.
...
The attention study is a 24-row, one-model diagnostic.  Attention mass is
averaged for reporting and is not a causal explanation.  Result replacement and
distractor removal alter content; they do not implement a query-specific
attention-logit mask.  These bounds justify deferring a beta sweep, not claiming
that provenance-aware attention can never help.
...
python -m ccpu paper2 analyze-public \
  --config configs/common/public_benchmarks.json \
  --cache-root /path/to/pinned-hf-parquet \
  --selection artifacts/paper2/public_benchmarks/data_v1/selection.jsonl \
  --output-dir artifacts/paper2/public_benchmarks/coverage_v1

python -m ccpu paper2 freeze-public-executable \
  --config configs/common/public_benchmarks.json \
  --cache-root /path/to/pinned-hf-parquet \
  --source-selection artifacts/paper2/public_benchmarks/data_v1/selection.jsonl \
  --per-benchmark 12 --output-dir artifacts/paper2/public_compute_v1/data

python -m ccpu paper2 freeze-public-bm25 \
  --config configs/common/public_benchmarks.json \
  --cache-root /path/to/pinned-hf-parquet \
  --source-selection artifacts/paper2/public_benchmarks/data_v1/selection.jsonl \
  --executable-selection artifacts/paper2/public_compute_v1/data/selection.jsonl \
  --output-dir artifacts/paper2/public_compute_v1/bm25_routes

python -m ccpu paper2 run-public-compute \
  --public-config configs/common/public_benchmarks.json \
  --cache-root /path/to/pinned-hf-parquet \
  --selection artifacts/paper2/public_compute_v1/data/selection.jsonl \
  --model-config configs/paper2/public_compute_qwen_xpu.json \
  --route-predictions artifacts/paper2/public_compute_v1/bm25_routes/predictions.jsonl \
  --condition cpu_bm25 --output-dir artifacts/paper2/public_compute_v1/cpu_bm25

python -m ccpu paper2 compare-generic-tools \
  --benchmark artifacts/paper2/generic_tools_v1/data/test.jsonl \
  --output-dir artifacts/paper2/generic_tools_v1/transport
...
diagnosis before any provenance-aware attention mechanism can be claimed.  The
bounded diagnosis does not support a fixed attention bias, so the runtime-copy
baseline remains the only new production recommendation.
Tokenizer-aware CPU routing strengthens the cheap baseline and can avoid most
model calls in a hierarchy, but its graph/engine-identity errors keep that
architecture diagnostic rather than production-qualified.
The four generic gateways now provide both the skeptical transport control and a
matched voluntary-call result.  Their visible schema is registry-invariant, but
Qwen invokes none of them on the executable public slice.  A disjoint BM25 CPU
route supplies meaningful automatic rescue, yet exact neural result use remains
only .683 and is weakest on CLUTRR.  This supports a bounded runtime-engineering
claim, not a fundamental interface claim: cheap routing can decide when to make
an exact result available, while runtime-copy is still required when the answer
