...
bias remains unlicensed.  A registered 2,253-row public suite then exposes a
formalization/backend gap: transferred T1/T2 engine recall is 0.139/0.016 and
the current exact contracts cover 0.163 of rows, although all 368 compatible
GSM8K traces execute exactly.  The Paper 3 gate remains \texttt{no\_go}.
\end{abstract}
...
The public suite now has a routing/formalization/backend coverage diagnostic,
but not yet a matched model-result table.  In particular, the current units
engine covers only a bounded simple-unit vocabulary, the date engine consumes
typed ISO operations rather than natural questions, the Horn engine does not
yet parse ProofWriter English, and the graph engine implements ISA closure
rather than CLUTRR kinship algebra.  Reporting oracle intent alone would hide
these measured gaps, so public model accuracy is deferred until matched
...
The four generic gateways now provide the skeptical transport baseline: with
oracle timing they are backend-equivalent to CogCop and their visible schema is
registry-invariant. A deeper invocation claim therefore requires a future
matched model-facing gain in recall, safe automatic rescue, JIT timing,
CONTINUE, or execution cost; transport parity alone supports no such claim.
